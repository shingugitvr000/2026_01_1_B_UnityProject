...
public class FruitGame : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (fruitTimer < 0 && fruitTimer > -2)      //타이머 시간이 0 과 -2 사이에 있을때 잰 함수를 초루 하고 다른 시간 대로 보낸다. 
        {
            SpawnNewFruit();
        ...
        if (currentFruit != null)                   //현재 생성된 과일이 있을 때만 처리 
        {
            ...
            Vector3 newPoisition = currentFruit.transform.position;                 //과일 위치 업데이트
            ...
            currentFruit.transform.position = newPoisition;                         //과일 좌표 갱신 
        }

        if (Input.GetMouseButtonDown(0) && fruitTimer == -3.0f)                 //마우스 좌 클릭하면 과이을 떨어트린다. && 타이머 시간은 -3.0f
        {
            DropFruit();
        }
    }
    ...
    void SpawnNewFruit()                            //과일 생성 함수
    {
        if (!isGameOver)                            //게임 오버가 아닐 때만 새 과일 생성 
        {
            ...
            Vector3 spawnPosition = new Vector3(worldPosition.x, fruitStartHeight, 0);          //X 좌표만 사용 하고 나머지는 설정한 값으로 한다. 
            ...
            if (rb != null)
            {
                rb.gravityScale = 0f;                       //시작 시에는 중력 스캐일을 0 으로 해준다. 
            }
        }
    }
    ...
    public void MergeFruits(int fruitType, Vector3 positioin)
    {
        ...
        {
            GameObject newFruit = Instantiate(fruitPrefabs[fruitType + 1] , positioin, Quaternion.identity);            //해당 타입 다음 번호를 생성 시킨다.
